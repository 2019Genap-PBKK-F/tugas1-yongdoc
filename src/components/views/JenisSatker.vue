...
var update = function(instance, cell, x, y, val) {
  axios
    .get('http://' + hostname + '/api/jenissatker/')
    .then((response) => {
      var index = Object.values(response.data[y])
      index[x] = val
      console.log(index)
      axios.put('http://' + hostname + '/api/jenissatker/' + index[0], {
        id: index[0],
        name: index[1]
      }).then((res) => {
...
var addrow = function(instance) {
  axios({
    method: 'post',
    url: 'http://' + hostname + '/api/jenissatker/',
    data: {
    }
  })
    .then((response) => {
...
var delrow = function(instance, id) {
  axios({
    ...
    .then((response) => {
      ...
      axios
        .delete('http://' + hostname + '/api/jenissatker/' + temp[0])
      console.log(response.data)
    })
}
...
export default {
  name: 'App',
  mounted: function () {
    axios.get('http://' + hostname + '/api/jenissatker/').then(res => {
      console.log(res.data)
      var jexcelOptions = {
        data: res.data,
        onchange: update,
