...
var update = function(instance, cell, x, y, val) {
  axios
    .get('http://' + hostname + '/api/aspek/')
    .then((response) => {
      var index = Object.values(response.data[y])
      index[x] = val
      console.log(index)
      axios.put('http://' + hostname + '/api/aspek/' + index[0], {
        id: index[0],
        aspek: index[1],
        k_aspek: index[2]
      }).then((res) => {
...
var delrow = function(instance, id) {
  axios({
    ...
    .then((response) => {
      ...
      axios
        .delete('http://' + hostname + '/api/aspek/' + temp[0])
      console.log(response.data)
    })
}
...
export default {
  name: 'App',
  mounted: function () {
    axios.get('http://' + hostname + '/api/aspek/').then(res => {
      console.log(res.data)
      var jexcelOptions = {
        data: res.data,
        onchange: update,
        ...
        columns: [
          ...
          { type: 'text', title: 'Komponen Aspek', width: '200px' }
        ]
      }
