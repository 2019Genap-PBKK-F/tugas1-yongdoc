...
var update = function(instance, cell, x, y, val) {
  axios
    ...
    .then((response) => {
      ...
      index[x] = val
      console.log(index)
      axios.put('http://' + hostname + '/api/periode/' + index[0], {
        id: index[0],
        name: index[1]
      }).then((res) => {
...
var delrow = function(instance, id) {
  axios({
    ...
    .then((response) => {
      ...
      axios
        .delete('http://' + hostname + '/api/periode/' + temp[0])
      console.log(response.data)
    })
}
...
export default {
  name: 'App',
  mounted: function () {
    axios.get('http://' + hostname + '/api/periode/').then(res => {
      console.log(res.data)
      var jexcelOptions = {
        data: res.data,
        onchange: update,
